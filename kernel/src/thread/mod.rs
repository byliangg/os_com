...
impl Thread {
    ...
    // No dead-code marker: the ext4 journal's commit-thread teardown
    // (`Journal::stop_commit_thread`, reached from `Ext4::drop`) is a non-ktest
    // user of `join`, so it is no longer dead in any build.
    #[track_caller]
    pub fn join(&self) {
