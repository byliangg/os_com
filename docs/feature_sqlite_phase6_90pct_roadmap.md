...
| P3b | ✅ **journal commit 块合并大写已落地（2026-06-11，SQLite 中性 243.97s，守底全绿保留）**。核实：checkpoints==commits 是良性计数语义；commit 桶（~90s）大头不在 journal 块写。**剩余 P3 项（sb 延迟/脏页索引）预期收益下调**——动手前必须先细分 ~90s 桶（writeback 时长/flush 次数与耗时/commit 等待分别计时），否则可能重蹈 P3-1 | — | 中性保留 | 0.5 天 | 已过门控 |
| P4 | ❌ **真 fdatasync 实测出局（2026-06-11，动手前核实，零实现成本）**：①`inode_tids` 已门控 `modified_blocks>0`——纯覆盖写 fsync 本就走已提交快路径 ②commit 由批量轮转驱动（rotations 4742/5131）非 fsync 驱动 ③`[ext4-fsync]` 实测整个 fsync 桶仅 28.6s（写回 21.5 + commit 等待 5.1 + flush 2.0），P4 撑死省 ~5s。instrumentation 已提交（d17f01e5a）| — | 出局 | 0.5 天 | "先核实再动手"再次兑现 |
| P5a | ✅ **已落地（2026-06-11，实测 234.9s = 21.92%，−4%）**：单遍探测捷径如设计；**预期 50-70s 实得 9s**——P1 块缓存后被跳过的树查已是 us 级，slow 桶剩余 ~240us/op 为结构性成本（convert+零填设备写+handle 框架+overlay 记录）。附带：`insert_extent` 损坏节点防御（generic/476 低频潜伏 panic→EIO 降级；4 轮判定与 P5a 无因果）+ crash runner 场景间 sleep（端口碰撞 flake 根治）。守底全绿 | P2 | ~~180–200s~~ **234.9s ✓** | 1 天 | 已过门控 |
| 收敛评估 | **曲线趋平：P1 −66% → P2 −46% → P5a −4%**。渐近线 ~25-30%：每 append 的结构性 journaled 工作（写时转换+零填+handle，~240us×549K≈130s）在"写时转换+无后台写回"平台约束下不可消除。**90% 在当前约束下不可达**；答辩口径建议 = 21-25% + 7.4× 提升 + 三 FS 三角归因 + 守底全绿 + 负结果方法论（delalloc/P3-1/P4 三次"先核实省实现"）。剩余可做：P5b 脏页索引（~12-15s → ~23-24%）、revoke 修复（保答辩）| — | — | — | 与学长对齐 |
| P5b | 脏页索引化 fsync（evict_range 全范围扫描 → 脏页索引集；~12–15s）| P5a | ~170–190s | 2–3 天 | 动共享 page_cache.rs，coherency 门控 |
