...
| P4 候选顺序调整 | **真 fdatasync 升为主攻**（砍 commit 次数 30–50%，对 ~90s 桶是次数级杠杆而非每次优化）；P3a 脏页索引 fsync 降级为"细分 profile 证实 evict 扫描占比后再做" | P3b | ~150–200s（25–34%）| 2–3 天 | 持久化语义（host-crash + fsync/flush 门控）|
