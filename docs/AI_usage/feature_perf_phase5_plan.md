...
核心方法论（学长指导 + DeepSeek + Claude Code 三方对齐结论）：
...
### Bug A 定位结果 + A1 修复（2026-06-06，instrument 收口）

经 4 轮只读 instrument（insert_extent / lock_block_group `#[track_caller]` / initial_write_alloc_bgid / ext_remove_blocks 现场值），**根因无歧义**：

- **现场**：`ext_remove_blocks: to=4294967295(u32::MAX) ex.first_block=0 pblock=0 actual_len=0 len≈43亿 total=524288`。
- **机制**：截断/删除 extent 时遇到**全零空 extent 槽（actual_len=0）**，调用方 `let end = ex.first_block + ex.get_actual_len() - 1 = 0+0-1` **u32 下溢成 u32::MAX**，当作释放范围上界 → `balloc_free_blocks` 释放范围越过整个 fs → `lock_block_group(16)` 越界 panic。panic 实际"保护"了数据；之前的边界兜底放行后该巨量 free 执行 → 释放海量有效块 → "database malformed"。

**Bug A 厘清为三层**：
| 层 | 性质 | 触发配置 | 状态 |
|----|------|----------|------|
| A1 | 零 extent → free 下溢 panic + 越界 free | page_cache=0 | ✅ 已修 |
| A2 | malformed 数据损坏（旧 Vec buffered 路径） | **仅 page_cache=0** | 遗留路径 bug，page_cache=1 不受影响 |
| B | 内核堆 OOM（buffered 内存无界增长） | page_cache=1 也有 | 真实配置 SQLite 的真正 blocker |

**关键**：真实应用默认配置 **page_cache=1 下无 A1/A2**，SQLite 正确跑过 test 100–170（SELECT 时间正常=数据未损坏），唯一 blocker 是 **Bug B（test 180 OOM）**。

**A1 修复（仅 `kernel/libs/ext4_rs/src/ext4_impls/extents.rs`，两层）**：
1. 主修：截断循环 `if ex.get_actual_len() == 0 { continue; }`——零长度 extent 映射 0 块、是无效槽，压缩掉它，从源头杜绝 `end` 下溢。
2. 防御：`ext_remove_blocks` 拒绝坏范围（`to<from` / `from<first_block` / `start+len>total` → warn 并 return 不释放），防越界 free / 损坏。
仅 `actual_len==0` 时触发，守底（无零 extent）零影响。编译 + ext4_rs 单测 29/29 通过。

**下一步优先级**：①守底验证 A1 不回退 → 提交；②**Bug B（page_cache=1 OOM）= 真实 SQLite blocker**，优先于 A2（page_cache=0 遗留路径）。

## 衍生任务：Bug B — buffered 写内核堆 OOM（2026-06-06 立项）

**症状**：page_cache=1 下 SQLite test 180（50万插入 w/3 索引）持续 ~600s 后 `Failed to allocate a large slot` + `Heap allocation error layout.size=0x1200(4608B)`——内核**堆**（非帧）的 large-slot 区耗尽。这是 page_cache=1 真实配置下 SQLite 的**真正 blocker**（A1/A2 不影响它，test 100–170 全过）。

**代码研究结论**：
- OOM 是 **large slot**（大块分配）。JBD2 每个 metadata 块在内存是 **`Vec<u8>` 4096B 副本**（`JournalBuffer.block_data` / `JournalCommitBlock.block_data`）= 正好 large slot → 强指向 **JBD2 内存里大量 4KB 块副本无界增长**。
- checkpoint 触发：commit 后 `free_after_commit < JOURNAL_CHECKPOINT_THRESHOLD(128)` 才 checkpoint（fs.rs:2122）+ fsync 时按 `checkpoint_depth` 触发（5299）——**基于磁盘 journal 空间/深度**。
- 候选无界结构：①`checkpoint_list: VecDeque<JournalTransaction>`（已提交待 checkpoint，各持 metadata 副本）②running transaction 的 `buffers: BTreeMap<u64, JournalBuffer>`（当前大事务累积脏块）③PageCache 脏页/索引 ④某缓存无上限。
- 待 instrument 确认：长事务（50万插入、中途不 fsync）下 checkpoint 整段不跑 → rotate 出的已提交事务 + 当前事务 buffers 是否无界堆积。

**修复路线（instrument-first，同 Bug A 方法论）**：
1. **加只读 instrument**：在 run_journaled_ext4 / commit 路径按计数门控 dump `checkpoint_list.len()` + 估算字节、running buffers count、（可及则）PageCache 页数，看哪个随时间线性增长到 OOM。
2. **复现**：SQLite page_cache=1，test 180 ~600s OOM（较慢，可考虑只跑到能看出增长趋势）。
3. **精确修**：若 checkpoint_list/running buffers 无界 → 基于内存/深度阈值主动 checkpoint（不只等磁盘空间），或 commit 后释放内存中 metadata 副本（数据已在磁盘 journal，checkpoint 时回读）——对齐 Linux JBD2"commit 后释放 buffer"语义。
4. **验证**：SQLite page_cache=1 不再 OOM（争取跑完 speedtest1 给 TOTAL）；完整守底回归全绿（crash/xfstests/concurrency 不回退）。

**风险控制**：instrument 只读零风险；改 checkpoint 触发/内存释放需谨慎（崩溃恢复正确性），守底 + crash matrix 门控；HEAD（含 A1）为安全基线。

### Bug B 定位 + 修复（2026-06-06，instrument 收口）✅ 已完成

instrument（`jbd2_memory_footprint()` 每 8192 ops dump）实测：`checkpoint_list` 内存涨到 **1.2 GB**（2208 已提交事务 × 各 ~4KB metadata 副本），`running_bytes` 始终 ≤1.5MB。根因：checkpoint 触发只看磁盘 journal 空间（`free<128`），长事务中途从不触发 → 内存无界。
