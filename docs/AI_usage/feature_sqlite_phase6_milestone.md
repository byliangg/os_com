...
**S6 是 Linux 对齐真功能**（unwritten extent 语义 + 写时预分配），不只是调参：ext4_rs 此前完全不支持 unwritten（file.rs:11），现在读返零/写转换/map 处理齐备，e2fsck/debugfs 互操作验证通过，crash matrix + host-crash 全过，AI 辅助审查记录支持主路径崩溃安全判断。新分配类写如归因预期拿到大头（CREATE INDEX ×3.4、ordered INSERT ×3.7、INSERT w/3idx ×2.4）。
...
- **S4 设计（fsync 安全点批量写回，下一步）**：给 `PageCacheBackend` trait 加 `write_pages_async`（默认逐页 = ext2 零影响），ext4 override = 连续脏页 run 一次 `ext4_map_blocks` + **已映射 run 免 JBD2 handle**（讨论记录：ordered 语义下纯数据写回可减少逐页 handle 开销）+ 一个大 bio；shared `page_cache.rs::evict_range` 按 run 分组。必保 C4 clamp 语义（`write_page_async` fs.rs:1158/1162）。⚠️ Phase 5 那次批量化只拿 3% = 只合并 bio 没干掉逐页 handle——本次必须干掉 handle。触碰 ext2 共享路径，默认实现必须保 ext2 行为不变。
...
| 日期 | Step | 改动概要 | 涉及文件 | 性能结果 | 守底 | commit |
|------|------|----------|----------|----------|------|--------|
| 2026-06-12 | C1 | **dio overwrite 共享锁并发（C 系列第一步，plan §C 系列）**：C0 调研钉死并发墙根因——fio numjobs 写**同一文件**（`-filename` 固定），per-inode 互斥锁横跨整个 O_DIRECT 路径含设备等待 → 单流封顶 ~2800；三证闭环（raw 线性 scale 117% / ext4 卡死 / journaled≈nojournal → 锁非 JBD2）。实现：`inode/dir correctness 锁 Mutex→RwMutex`（既有持锁点全 `.write()` 语义不变）；`write_direct_at` pc=0 下先持 **shared** 做 overwrite 判定（判定在锁内，TOCTOU 安全：所有映射变更路径独占）、命中即 shared 并行提交、未命中转独占 prepare；`read_direct_at` pc=0 shared / pc=1 独占（保守）。= Linux shared `i_rwsem` dio overwrite 的对应实现。另：sweep 脚本加 `SWEEP_GROUPS` 组过滤 | `kernel/src/fs/ext4/fs.rs`、`test/initramfs/src/benchmark/fio/run_parameter_sweep_summary.sh` | **F 组：write nj2 2708→6024（65%→165%）、nj4 2724→5139（63%→187%）——并发墙拆除、反超 Linux 与 raw；read nj4 3640→13200**。SQLite 255.3s/ratio 22.65%（pc=1 不走 shared 路径，宿主噪声轮）| **C2 链全绿**：fio nj1 守底（三轮裁决：47.65% 噪声轮 → 118.36%/88.26% 双侧同降轮，ratio 健康，C1 无伤 nj1）；phase2 concurrency 7/7 + concurrency xfstests 10/10（本改动专属把关）；crash 18/18 + host-crash 4/4 + fsync_flush + phase6_with_guard 四包全过 | 待提交 |
