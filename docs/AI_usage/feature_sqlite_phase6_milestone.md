...
**S6 是 Linux 对齐真功能**（unwritten extent 语义 + 写时预分配），不只是调参：ext4_rs 此前完全不支持 unwritten（file.rs:11），现在读返零/写转换/map 处理齐备，e2fsck/debugfs 互操作验证通过，crash matrix + host-crash 全过，DeepSeek V4 Pro 定点审计判定主路径崩溃安全 SOUND。新分配类写如归因预期拿到大头（CREATE INDEX ×3.4、ordered INSERT ×3.7、INSERT w/3idx ×2.4）。
