...
| **S6** | **物理预分配 + 追加快路径（攻 32%）**：`WRITE_PREALLOC_BLOCKS`/`SMALL_WRITE_PREALLOC_BLOCKS`（file.rs:14-15，现 =1）顺序追加时提到 32–64（管道现成，868-975）；给"已映射但越 EOF"的追加开轻 handle 快路径（只 journal inode size，跳过分配/映射重查）| 32%（写时每页分配 → 每段一次）| 写时分配，**不延迟、不途中写回**（安全）；唯一实质风险 = unwritten extent（ext4_rs 可能不支持，e2fsck 可能告警）→ **先做 30 分钟实验**（改常量→写→崩→e2fsck），退路：fsync/close 时 trim 多余块 或 补 unwritten 支持 | 中（有退路）|
