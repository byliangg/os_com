...
- **本轮 nj1 行（write 49.84% 等）连同 raw（72.64%/39.62%）整体异常偏低**：raw 不经 ext4 代码同样大降 → 宿主环境噪声。nj1 守底经三轮独立裁决收口（`run_ext4_summary.sh`，1M nj1 同口径）：噪声轮 47.65% → **双侧同降轮 write 118.36%（Aster 1631 / Linux 1378）、read 88.26%（1789/2027）**——Linux 侧绝对值同步腰斩证实环境劣化、双侧同降下 ratio 健康，**C1 对 nj1 无损**（日志 `/tmp` 守底链 + `benchmark/logs/ext4*`）。
