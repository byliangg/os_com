...
本文件是 benchmark 与复现的**唯一指引**（原 `environment.md` 已并入本节后删除）：§0 最新结果、本节环境准备、§4/§6 各 benchmark 精确跑法。
...
## 6.7 SQLite 真实应用 benchmark（speedtest1，Phase 5 起 / Phase 6 主线）

赛题要求"用 SQLite 等真实应用测文件系统真实性能"。SQLite 把整个 DB 当文件系统上**一个文件**，跑时是 buffered I/O + 每事务 fsync（COMMIT）+ 随机小块读写的混合负载——fio O_DIRECT 守底测不到。**这是 Phase 6 的主优化对象。**

### 运行方法（宿主机，有 docker；脚本自管容器）
...
- **入口**：`test/initramfs/src/benchmark/sqlite/run_sqlite_summary.sh`（宿主机跑，内部 `docker run` 起 `asterinas/asterinas:0.17.0-20260227`，已带 `BENCH_ENABLE_KVM=1 BENCH_ASTER_NETDEV=tap BENCH_ASTER_VHOST=on BENCH_ASTER_SCHEME=null`）。
- **内部**：对每个 page_cache 配置调 `bench_linux_and_aster.sh sqlite/ext4_speedtest1 x86_64` → guest 跑 `sqlite-speedtest1 --size 1000 /ext4/test.db`（`ext4_speedtest1/run.sh`），Asterinas 与 Linux **同口径同轮对照**，drop_caches 公平基线。
- **可调 env**：
  - `PAGE_CACHE_LIST`（默认 `1 0`）——`1`=PageCache buffered 路径（**真实应用默认配置**），`0`=旧 Vec buffered 路径（有 A2 corruption 遗留，仅历史对照）。**只想跑真实配置：`PAGE_CACHE_LIST=1`**。
  - `SQLITE_SIZE`（默认 `1000`）——注意 `--size` 不缩放插入行数（100/1000 均固定 50 万行），不能靠减 size 提速。
  - `LOG_LEVEL`（默认 `error`；profile 时设 `warn`）、`LOG_DIR`、`IMAGE`。
...
### profile 跑法（Phase 6 Step 0，复用 Phase 5 四层 profile，不重造）

加内核命令行 `ext4fs.phase2_profile=1` + `LOG_LEVEL=warn`，跑同一 SQLite，收割 `[ext4-direct-write]` / `[ext4-profile] direct-read` / `[block-profile]` + BUFW probe（`write_at_page_cache` 计数/耗时，Phase 5 末尾加的）。**铁律：先 profile 再优化**（Phase 5 写回批量化未 profile 先动手只拿 3% 被回退的教训）。
...
本文三处副本须保持同步（改一处同步另两处）：
- 根目录工作副本：`benchmark.md`
- 仓库内（benchmark 目录）：`asterinas/benchmark/benchmark.md`
- 仓库内（docs 目录）：`asterinas/docs/benchmark.md`

相关阶段文档：
- fio O_DIRECT 守底 / 三瓶颈：`fio_direct_parameter_sweep_report.md`、`docs/fio_direct_parameter_sweep_report_phase5.md`
- Phase 5 性能优化：`feature_perf_phase5_plan.md` / `feature_perf_phase5_milestone.md`
- **SQLite 真实应用（§6.7 跑法）**：`sqlite_benchmark_report.md`（报告）、`feature_sqlite_phase6_plan.md` / `feature_sqlite_phase6_milestone.md`（Phase 6 主线）
