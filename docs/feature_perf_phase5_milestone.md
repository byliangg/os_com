...
| 2026-06-06 | SQLite 真实应用驱动修复两崩溃 bug：**A1** 截断零长度 extent 致 free 下溢 panic（`extents.rs` 跳过+防御）；**B** `checkpoint_list` 无界涨 1.2GB 致堆 OOM（`fs.rs` 加 `JOURNAL_CHECKPOINT_MAX_DEPTH=64` depth 触发）。page_cache=1 下 SQLite speedtest1 **端到端跑通**（TOTAL 4773s/Linux 60s=1.26%，integrity_check 通过）。守底全绿（A1 已验，B 验证中） | extents.rs, fs.rs, journal.rs, sqlite_benchmark_report.md |
| 2026-06-07 | buffered write 优化（profile 驱动）：BUFW probe 实测写时逐 write journaled prepare = 83%（9M×428µs），绝大多数是已分配块覆盖写。修：`write_at_page_cache` 覆盖写快路径（全映射+不扩展 → 跳过 run_journaled_ext4 + 节流 mtime + 页缓存写）。SQLite TOTAL 4773s→2022s（**2.36×**），ratio 1.26%→2.97%，integrity_check 过。守底全绿。剩余慢项=追加/分配（delalloc 待定） | fs.rs |
