...
Options:
  ...
  --timeout <dur>       Timeout passed to phase4 runner (default: ${TIMEOUT_S})
...
if command -v chown >/dev/null 2>&1; then
  chown -R "${HOST_UID}:${HOST_GID}" stage4_ext4_logs_part3 >/dev/null 2>&1 || true
fi
CMD
)
RUN_CMD=${RUN_CMD/TIMEOUT_PLACEHOLDER/timeout ${TIMEOUT_S} tools/ext4/run_phase4_part3.sh}
