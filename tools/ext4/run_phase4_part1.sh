...
LOG_DIR=${LOG_DIR:-"${ROOT_DIR}/benchmark/logs/part1"}
mkdir -p "${LOG_DIR}" "${LOG_DIR}/lmbench"

INITRAMFS_IMG=${INITRAMFS_IMG:-"${ROOT_DIR}/benchmark/assets/initramfs/initramfs_phase4_part1.cpio.gz"}
...
if [ ! -f "${INITRAMFS_IMG}" ]; then
  "${ROOT_DIR}/tools/ext4/prepare_phase4_part1_initramfs.sh" "${BASE_INITRAMFS}" "${INITRAMFS_IMG}"
fi
...
run_xfstests_mode() {
  ...
  local qlog="qemu.log"
  if [ -f kernel/qemu.log ]; then
    qlog="kernel/qemu.log"
  fi
  if [ -f qemu.log ]; then
    qlog="qemu.log"
  fi

  cp -f "${qlog}" "${log_file%.log}.qemu.log" || true

  echo "[DONE] mode=${mode} rc=${rc} log=${log_file}"
  rg -n "mode\\tpass\\tfail|${mode}\\t|xfstests ${mode} passed|xfstests phase3_base passed|All syscall tests passed|Error: xfstests failed|xfstests case done" "${qlog}" | tail -n 60 || true

  if [ "${allow_fail}" = "0" ] && [ ${rc} -ne 0 ]; then
    return ${rc}
  fi
  return 0
}
...
run_lmbench_regression() {
  ...
  for bench in "${benches[@]}"; do
    ...
    mkfs.ext4 -F -b 4096 test/initramfs/build/ext2.img >/tmp/mkfs_ext4_phase4_part1.log 2>&1
    ...
    local key=""
    case "${bench}" in
      lmbench/ext4_vfs_open_lat)  key=$(grep -E "Simple open/close" "${qlog}" | tail -n 1 || true) ;;
      lmbench/ext4_vfs_stat_lat)  key=$(grep -E "Simple stat" "${qlog}" | tail -n 1 || true) ;;
      lmbench/ext4_vfs_fstat_lat) key=$(grep -E "Simple fstat" "${qlog}" | tail -n 1 || true) ;;
      lmbench/ext4_vfs_read_lat)  key=$(grep -E "Simple read" "${qlog}" | tail -n 1 || true) ;;
      lmbench/ext4_vfs_write_lat) key=$(grep -E "Simple write" "${qlog}" | tail -n 1 || true) ;;
      lmbench/ext4_create_delete_files_0k_ops|lmbench/ext4_create_delete_files_10k_ops)
        key=$(grep -E "^[0-9]+k[[:space:]]+[0-9]+[[:space:]]+[0-9]+[[:space:]]+[0-9]+" "${qlog}" | tail -n 1 || true)
        ;;
      lmbench/ext4_copy_files_bw)
        key=$(grep -E "lmdd result: .* MB/sec" "${qlog}" | tail -n 1 || true)
        ;;
    esac
...
LMB_SUMMARY="${LOG_DIR}/lmbench/phase4_part1_lmbench_summary_${TS}.tsv"

# phase4_good is baseline/observation in part1; keep non-fatal.
run_xfstests_mode phase4_good "${PHASE4_GOOD_THRESHOLD}" "${PHASE4_LOG}" 1

# phase3_base guard is mandatory in part1.
run_xfstests_mode phase3_base 90 "${PHASE3_LOG}" 0

# LMbench 8 guard is mandatory in part1.
run_lmbench_regression "${LMB_SUMMARY}"

echo "[DONE] phase4_part1 run finished"
