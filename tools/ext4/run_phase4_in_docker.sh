...
run_part3_with_flags() {
  env \
    ...
    XFSTESTS_SINGLE_TEST="${XFSTESTS_SINGLE_TEST}" XFSTESTS_CASE_TIMEOUT_SEC="${XFSTESTS_CASE_TIMEOUT_SEC}" \
    ...
    RUN_PHASE3_BASE="${RUN_PHASE3_BASE}" RUN_LMBENCH="${RUN_LMBENCH}" \
...
case "${PHASE4_DOCKER_MODE}" in
  phase4_good)
    ...
    RUN_CRASH_SUITE=0 RUN_PHASE4_GOOD=1 RUN_PHASE3_BASE=0 RUN_LMBENCH=0 run_part3_with_flags
    ;;
  phase3_only)
    echo "[INFO] mode=phase3_only (only xfstests phase3_base)"
    RUN_CRASH_SUITE=0 RUN_PHASE4_GOOD=0 RUN_PHASE3_BASE=1 RUN_LMBENCH=0 run_part3_with_flags
    ;;
  lmbench_only)
    echo "[INFO] mode=lmbench_only (only lmbench regression)"
    RUN_CRASH_SUITE=0 RUN_PHASE4_GOOD=0 RUN_PHASE3_BASE=0 RUN_LMBENCH=1 run_part3_with_flags
    ;;
  phase4_with_guard)
    echo "[INFO] mode=phase4_with_guard (phase4_good + phase3_base)"
    RUN_CRASH_SUITE=0 RUN_PHASE4_GOOD=1 RUN_PHASE3_BASE=1 RUN_LMBENCH=0 run_part3_with_flags
    ;;
  part3_full)
    echo "[INFO] mode=part3_full (crash + phase4_good + phase3_base + lmbench)"
    RUN_CRASH_SUITE=1 RUN_PHASE4_GOOD=1 RUN_PHASE3_BASE=1 RUN_LMBENCH=1 run_part3_with_flags
    ;;
  *)
    echo "Error: unsupported PHASE4_DOCKER_MODE=${PHASE4_DOCKER_MODE}" >&2
    echo "Supported: phase4_good | phase3_only | lmbench_only | phase4_with_guard | part3_full" >&2
    exit 3
    ;;
esac
EOF
